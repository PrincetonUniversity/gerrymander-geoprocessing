...
# -*- coding: utf-8 -*-
"""
Created on Thu Dec 13 10:45:47 2018
...
path = '/Volumes/GoogleDrive/Team Drives/princeton_gerrymandering_project/mapping/PA/Voter Roll/'

df_raw = pd.DataFrame()
files = os.listdir(path)
#go through files in directory and take only "missed.csv" files
for name in files:
    if name.__contains__('missed'):
        ...
        df_temp = pd.read_csv(raw_path, names=raw_cols, header=0)
        df_temp = df_raw.set_index('id')
        df_temp['address'] = df_raw['address'].str.strip()
