...
try:
    ...
    csv_col = ['locality', 'type', 'aggregate on', 'to_df path', 'to_df cols',\
               'from_df path', 'from_df cols', 'round', 'preserve']
    
    csv_list = ['to_df cols', 'from_df cols', 'round', 'preserve']
    ...
    for i, _ in csv_df.iterrows():
        ...
        try: 
            ...
            round_cols = csv_df.at[i, 'round']
            preserve_cols = csv_df.at[i, 'preserve']
            ...
            for ix, elem in enumerate(to_cols):
                adjust_cols.append((elem, from_cols[ix], round_cols[ix], \
                                    preserve_cols[ix]))
                
            # Get to path and from path
            to_path = ht.default_path(csv_df.at[i, 'to_df path'], local, \
                                      direc_path)
            from_path = ht.default_path(csv_df.at[i, 'from_df path'], local,\
                                        direc_path)
            ...
            new_df_to = ht.interpolate_aggregate(df_to, df_from, adjust_cols,\
                                             csv_df.at[i, 'type'])
